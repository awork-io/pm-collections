...
export class Variable implements IVariable {
  ...
  static resolve(obj: any) {
    if (obj instanceof Variable || !this.validate(obj)) return obj;
    return new Variable(obj);
  }
  static validate(obj: any) {
    return !!(obj && (obj.key || obj.value || obj.type || obj.system));
  }
}
...
export class VariableList extends PropertyList<Variable> {
  ...
  static resolve(arr: any) {
    if (arr instanceof VariableList || !this.validate(arr)) return arr;
    return new VariableList(arr);
  }
  static validate(arr: any) {
    return !!(arr && arr.every((obj: any) => Variable.validate(obj)));
  }
}
